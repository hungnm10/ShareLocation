 ...
 */
public class CreateChannelAsync extends AsyncTask<Channel,Void,String> {
    private IAsynTaskDelegate interfa;
    private ProgressHUD hud;
    private FragmentManager fManager;

    public CreateChannelAsync(ProgressHUD aHud, FragmentManager aManger,
                              IAsynTaskDelegate anInterface) {
        this.hud = aHud;
    ...
    protected String doInBackground(Channel... params) {
        String result = CoreService.getInstance().createChannel(params[0]);
        return result;
    }
    @Override
    protected void onPostExecute(String result) {
        ...
        if (result != null) {
            ...
            try {
                if (obj == null) {
                    SomeDialog dialog = new SomeDialog("Error",
                            Constant.INTERNET_CONNECT_ERROR, "OKay", "",
                            null);
            ...
            SomeDialog dialog = new SomeDialog("Login",
                    Constant.INTERNET_CONNECT_ERROR, "OK", "", null);
            dialog.show(fManager, "dialog");
        }
    }
    @Override
